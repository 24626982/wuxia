# 劍譜疑雲

Godot 4.7.2 原生武俠懸疑遊戲。已整合 v1.0 JSON 劇本：序章、西院查訪、宿舍、城鎮、東竹亭、聽風崖、議事廳重訪與終幕。

以 Godot 開啟 `project.godot` 後按 F5，或直接執行 `builds/windows/Wuxia.exe`。台詞與素材均在本機，不需要網路。跨地圖保留本次遊戲進度；目前沒有磁碟存檔，重新啟動會重新開始。

## 操作與流程

- WASD／方向鍵移動，R 回目前地圖抵達點。
- 靠近人物或標記按 E／空白鍵；對話中以 E／空白鍵／Enter 繼續。
- ↑↓／W、S 選取選項，Enter 確認，或用滑鼠點選。1、2 可直接選前兩項；長選單可捲動。
- Esc 取消當前對話；連續劇情與衝突事件會回復到事件開始前的狀態。
- 右下角「線索」開啟線索簿，可捲動查看已取得的線索、證詞與取得方式；按關閉或 Esc 返回。開啟時暫停角色操作，對話期間隱藏按鈕。紀錄隨本次遊戲跨地圖保留，取消對話不會留下未完成的線索。
- 「線索」左側的「記錄」按鈕會下載 UTF-8 文字檔，列出本次遊戲開始時間，以及每次與人物或調查物件互動的時間。Windows 版會開啟「另存新檔」視窗，可自行選擇路徑與檔名；網頁版由瀏覽器依下載設定處理儲存位置。
- 戰鬥採必勝的回合選單：選擇對手出劍，依回合、體力、倒下事件播放原稿台詞。

依右上角目標前進：掌門 → 阿棠 → 秦川（交談後整理線索）→ 左側石橋 → 西院三位證人與周伯 → 宿舍石安 → 小滿的空床 → 城鎮告示與小滿哥哥 → 東竹亭第三盞燈 → 回宿舍探陳白 → 聽風崖 → 議事廳 → 聽風崖對質 → 議事廳覆命。秦川對話完成後即開放西院查訪，不必先下山。西院南側石路以與原有庭園一致的灰色石牆封閉，出入口為右側石橋與北側宿舍門。序章完成前可自由走訪地圖，西院證詞與後續主線尚未開放。夜間事件由「等候入夜」互動點觸發。聽風崖可先查竹管線索，再決定是否搜尋藏處；知道藏處後，對質前仍可選擇是否先取回劍譜。

| 地圖 | 入口 |
| --- | --- |
| 西院 | 庭院左橋 |
| 議事廳 | 庭院掌門後方向北 |
| 弟子宿舍 | 西院周伯後方向北 |
| 山腳城鎮 | 庭院南方石階 |
| 東竹亭 | 城鎮東南出口 |
| 聽風崖 | 東竹亭北路 |

已補齊 12 位新 NPC 的獨立正面、背面、左側、右側透明圖，接入靠近玩家時的轉身，並加入對話立繪。東竹亭與聽風崖已換成與既有場景一致的像素美術，調整互動位置及碰撞。四方向站姿不包含逐格行走或戰鬥動畫。素材及生成提示見 [assets/NPC_DIRECTIONS.md](assets/NPC_DIRECTIONS.md) 與 [assets/PROMPTS.md](assets/PROMPTS.md)。

右上角逐步提示下一位人物、線索與出口；互動點使用對話、放大鏡、月亮及任務標記。西院先查訪沈禾、白芷、顧衡，再找周伯核對；先找周伯不會計入證詞。宿舍尚未解鎖的物件會提示先問石安。介面不顯示交鋒次數。

## 劇本與判定

九份台詞檔位於 `data/*.json`，沿用專案原來的資料位置。`data/game_logic.json` 定義事件、證據、衝突、搜查、戰鬥與結局表。

- `scripts/story_rules.gd`：共用條件運算與即時計算的 `fight_count`、`route`。
- `scripts/dialogue.gd`：條件台詞、第一個符合的變體、條件選項、回應後動作、捲動選單。
- `scripts/story_director.gd`：N1–N5、第三次錯誤、耐心、證據、戰鬥、暴力回響、終幕換圖。
- `scripts/courtyard.gd`：互動點、任務提示、原有序章與西院流程。

`approach`、`clue`、`trust_guard` 不影響正式結局。結局只按五次衝突結果與 `got_manual` 判定。舊探索鍵及 `observe`／`verify` 值保留；宿舍床鋪與藥袋在 N1 完成後開放。「東竹亭尚未開放」提示已改為實際路線。

六種結局皆可自然達成。`hide_spot` 只表示知道藏處；N5 對質前選擇先取回劍譜才會設定 `manual_secured = true`。先取回後，即使談判失敗也能保住劍譜；未先取回時，談判成功仍能由嚴承交回，失敗則失去劍譜。「重要的寶物」路線：N1–N4 全部說理，N5 選擇暫不取走劍譜、跟他談，耐心耗盡後失去劍譜；五次衝突皆維持說理結果。

## 驗證與匯出

遊戲啟動時會先進入「初入江湖」：輸入主角姓名（最多 12 字）、選擇男／女，再按「踏入江湖」。對話顯示自訂姓名；女性主角使用獨立立繪及正面、背面、左側、右側四方向素材，換地圖會保留姓名與性別。姓名與性別目前與劇情進度一樣保留於本次遊戲，不另提供存檔功能。

女性素材以內建 imagegen 生成，原始 PNG 及透明度保持不變；提示與來源見 `assets/characters/hero-female-prompts.md`。男女開場預覽按可見角色邊界統一為 210 像素高，避免透明留白造成縮放差異。`character_creation_test` 驗證空白姓名、角色選擇、對話、四方向腳底對齊、跨地圖及開場進入庭院。

以 `godot --headless --path . --script res://tests/<檔名>.gd` 執行：

- `story_campaign_test`：序章至終幕、四種前期組合、說理／暴力／混合、搜尋與未搜尋、耐心耗盡。六種結局皆由完整劇情流程驗證。
- `ending_fuzz_test`：200 場固定種子的隨機通關，驗證六種結局可達、對質前搜查與終幕完成。
- `ending_cancel_test`：120 場隨機取消與重試，驗證劇情回滾、終幕待播狀態與最終收尾。
- `ending_regression_test`：對質前搜查、略過、條件限制與取消回滾，以及結局表缺項時的備援播放（預期輸出兩筆缺項錯誤紀錄）。
- `story_progression_test`：序章進度限制、床鋪調查後才開放證詞、完成 N2 後的指引，以及取回劍譜選擇的取消回復。
- `story_events_test`：觸發、解鎖、錯誤上限、離開重試、取消回復、互動接近位置與障礙、終幕實際換圖。
- `choices_test`、`residence_test`：原有序章分支與西院查訪。
- `movement_test`、`maps_test`、`expansion_test`、`seven_maps_test`：移動、碰撞、七張地圖往返。
- `offline_audit`：本地引用與離線匯出設定。
- `guidance_test`、`directional_cast_test`、`map_access_test`：周伯計數、逐步指引、全角色立繪、12 位 NPC 四方向切換與腳底對齊、地圖互動點可達性。

`tests/preview_campaign.gd` 需以有繪圖的 Godot 執行，輸出地圖與長選單截圖。使用 `tools/build_windows.ps1 -GodotPath '本機 Godot 執行檔路徑'` 重新匯出。`tests/pack_test.gd` 可搭配 `--main-pack builds/windows/Wuxia.exe --script D:/wuxia/tests/pack_test.gd` 檢查內嵌資料包。

## GitHub 網頁版

網頁版的啟用與自動更新方式請見 [GitHub Pages 說明](docs/github-pages.md)。
