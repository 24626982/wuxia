...
func run() -> void:
	...
	to_choice()
	check(bed_choice() >= 0, "Completing N1 is enough to reassure the brother about the bed")
	world.dialogue.cancel()
	...
	check("崖上還沒有人" in world.dialogue.body.text, "Known hiding spot starts at the tube before Yan arrives")
	to_choice()
	check(world.dialogue.current_choices.size() == 2, "Tube offers take or leave choices")
	world.dialogue.choose(0)
	to_choice()
	check(world.story_flags.get("took_manual_first", false) and world.dialogue.current_choices.size() == 2, "Taking the manual leads to the N5 fight-or-talk choice")
	world.dialogue.cancel()
	check(world.story_flags == before and not world.director.busy and world.player.movement_enabled, "Cancelling N5 rolls back and unlocks movement")
	world.director.enter_map()
	to_choice()
	world.dialogue.choose(1)
	to_choice()
	check(not world.story_flags.get("took_manual_first", true), "Leaving the manual records the risk before confrontation")
	world.dialogue.cancel()
	check(world.story_flags == before, "Cancelling retry restores the entry flags")
	world.story_flags = {"n1_result": "fight", "n2_result": "fight", "n3_result": "fight", "n4_result": "fight", "suspect_known": true, "monologue": "rules", "chen_bai_nails": true, "explore_wind_cliff_record": "observe"}
	world.director.entry_seen.clear()
	world.director.enter_map()
	check(world.dialogue.waiting_for_choice and world.dialogue.current_choices.size() == 2, "Unsearched N5 warns before committing to the confrontation")
	check(world.dialogue.current_choices[0].label == "先退開，搜查竹管。", "Warning offers a visible search-first exit")
	world.dialogue.choose(0)
	await process_frame
	check(not world.director.busy and not world.director.entry_seen.has("n5"), "Search-first exit leaves N5 available for retry")
	world.director.enter_map()
	world.dialogue.choose(1)
	check("嚴承從崖邊" in world.dialogue.body.text and not world.story_flags.get("took_manual_first", true), "Continuing without the hiding spot leaves the manual with Yan")
	world.dialogue.cancel()
	world.queue_free()
	await process_frame
